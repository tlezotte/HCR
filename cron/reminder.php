...
$sql = $dbh->prepare("SELECT p.id, p.purpose, p.req, p.reqDate, a.app1, a.app1Date, a.app2, a.app2Date, a.app3, a.app3Date, a.app4, a.app4Date, a.staffing, a.staffingDate,
					   ...
					   TO_DAYS(NOW()) - TO_DAYS(a.app4Date) AS curApp4Staffing
...
function sendMail($sendTo,$PO_Level,$REQUEST_ID,$PURPOSE,$DAYS,$REQUESTER) {
	...
	$URL = $default['URL_HOME']."/PO/detail.php?id=".$REQUEST_ID."&approval=".$PO_Level;
	$URL2 = $default['URL_HOME']."/PO/cancelSlip.php?id=".$REQUEST_ID;
...
while($sth->fetchInto($PO)) {
	...
	} else if (isset($PO['app2']) and $PO['app2'] != '0' and is_null($PO['staffingDate']) and $PO['curApp2Staffing'] >= 2 and $PO['curApp2Staffing'] % 2 == 0) {
 		$APP = $dbh->getRow("SELECT eid, email
							 FROM Standards.Employees
