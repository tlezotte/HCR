...
if ($_GET['action'] == "my" and $_GET['view'] == "all") {
	...
	$view_all = htmlspecialchars($_SERVER['PHP_SELF']."?action=my&access=".$_GET['access']);
	$view_gif = 'VIEW OPEN';
	...
	$view_all = htmlspecialchars($_SERVER['PHP_SELF']."?action=my&view=all&access=".$_GET['access']);
	$view_gif = 'VIEW ALL';
} else {
	$where_clause = "p.po IS NULL";
	$view_all = htmlspecialchars($_SERVER['PHP_SELF']."?view=all");
	$view_gif = 'VIEW ALL';
}
...
/* SQL for PO list */
$po_sql = "SELECT p.id, p.po, p.purpose, p.reqDate, p.req, p.sup, p.total, a.app1, a.app1Date, a.app2, a.app2Date, a.app3, a.app3Date, a.app4, a.app4Date, a.staffing, a.staffingDate, p.status
			 FROM PO p, Authorization a
			 WHERE p.id = a.request_id AND $where_clause $where_status
          ...
          <table border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <td><select name="select" id="select" onChange="this.form.submit();">
                ...
                <option value="5" <?php if ($_GET['access'] == '5') { echo "selected"; } ?>>Staffing</option>
              </select>
...
while($po_sth->fetchInto($PO)) {
	...
	} else if ($_GET['action'] == "my" AND $_GET['access'] == '5' AND !isset($PO['staffingDate'])) {
		$approval_option = "&approval=staffing";
	} else {
		$approval_option = "";
	}										
?>  
  <tr bgcolor="FFFFFF">
    <td bgcolor="#<?= $row_color; ?>"><table border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td width="18"><img src="../../images/<?= $position; ?>" width="18" height="20" border="0" align="absmiddle"></td>
        <td nowrap><?= ucwords(strtolower(stripslashes($PO[purpose]))); ?></td>
      </tr>
    </table>      
    </td>
  </tr>
