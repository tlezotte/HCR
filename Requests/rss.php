...
$approved_query = <<< SQL
	SELECT p.id, p.purpose, p.req, p.company, a.staffing, a.staffingDate
	FROM $DATABASE p, Authorization a
	WHERE p.id = a.request_id AND a.type = '$DATABASE' AND a.staffingDate IS NOT NULL
	ORDER BY a.staffingDate DESC
	LIMIT $rss_items
SQL;
...
$denied_query = <<< SQL
	...
	FROM $DATABASE p, Authorization a
	WHERE p.id = a.request_id AND a.type = '$DATABASE' AND (a.app1yn = 'no' OR a.app2yn = 'no')
...
while($denied_sth->fetchInto($DENIED)) {
	$title = stripslashes($DENIED['purpose']);
	$company = ucwords(strtolower($COMPANY[$DENIED[company]]));
	$author = ucwords(strtolower($EMPLOYEES[$DENIED[req]]));
